...
We approximate standard error as $s = \sqrt{\Sigma(X_i-\overline{X})^2/(n-1)}$
So our confidence interval is $\overline{X}\pm t_{n-1, \alpha/2} * s/\sqrt{n}$
...
for (n in num_repetitions){
  for (size in sample_sizes){
    ...
    results <- rbind(results, data.frame(
        ...
        Size_n = size,
        Alpha = alpha,
        Method = c("1.Exact", "2.Norm(K)", "3.Norm(Inv)", "4.Student-t"),
...
colnames(stats_wide_all) <- gsub("AvgLength.", "", colnames(stats_wide_all))

stats <- stats_wide_all[order(stats_wide_all$Reps_m, stats_wide_all$Size_n, stats_wide_all$Alpha), ]

print(stats_wide_all, row.names = FALSE)
```

```{r}
coverage_data <- results[, c("Reps_m", "Size_n", "Alpha", "Method", "Coverage")]

cov_wide_all <- reshape(coverage_data, 
                        idvar = c("Reps_m", "Size_n", "Alpha"), 
                        timevar = "Method", 
                        direction = "wide")

colnames(cov_wide_all) <- gsub("Coverage.", "", colnames(cov_wide_all))

cov_wide_all$Target <- 1 - cov_wide_all$Alpha

cols <- c("Reps_m", "Size_n", "Alpha", "Target", setdiff(names(cov_wide_all), c("Reps_m", "Size_n", "Alpha", "Target")))
cov_wide_all <- cov_wide_all[, cols]

cov_wide_all <- cov_wide_all[order(cov_wide_all$Reps_m, cov_wide_all$Size_n, cov_wide_all$Alpha), ]

print(cov_wide_all, row.names = FALSE)
```
